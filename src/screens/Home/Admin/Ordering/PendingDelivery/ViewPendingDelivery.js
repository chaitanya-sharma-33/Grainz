...
class ViewPendingDelivery extends Component {
  ...
  getOrderFun = () => {
    ...
    getOrderByIdApi(productId)
      .then(res => {
        ...
        let finalOrderData = sortedList.map((item, index) => {
          return {
            ...item,
  ...
  isCheckedEditableStatusFun = () => {
    const {flagAllStatus, saveStatus} = this.state;
    if (flagAllStatus) {
      ...
      this.setState(
        ...
        () => (saveStatus ? this.props.navigation.goBack() : null),
      );
    } else {
      const {pageData} = this.state;
      const finalStatus = pageData.orderItems.some((item, index) => {
  ...
  hitProcessOrderApi = () => {
    ...
    let payload = {
      ...
      isChecked: reviewStatus ? switchValueAll : false,
      checkedBy: userName,
    };
  ...
  saveFunInventoryItemThird = () => {
    ...
    let payload = {
      ...
      pricePaid: Number(finalPriceActual),
    ...
    console.log('payload-->INVENTOY', payload);
    processPendingOrderItemApi(payload)
      .then(res => {
        this.setState(
          {
                  ...
                  <View>
                    ...
                    {pageData && pageOrderItems.length > 0 ? (
                      pageOrderItems.map((item, index) => {
                        console.log('item->pageOrderItems', item);
                        return (
                          <View key={index}>
                            {/* <View
                              style={{
                    ...
                    <View style={styles.secondContainer}>
                      ...
                      {pageOrderItems.map((item, index) => {
                        ...
                        const volume = item.inventoryMapping
                          ? item.inventoryMapping.volume
                          : item.grainzVolume;
                        const modalQuantityDelivered = item.quantityDelivered;
                        const modalUserQuantityDelivered =
                          item.userQuantityDelivered;
                        const modalQuantityInvoiced = item.quantityInvoiced;
